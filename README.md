...
endpoint and is intentionally not implemented.
...
same-speaker turns, preserves utterance text byte-for-byte, and reports unmapped
labels on stderr. It touches no Drive and spends nothing.
...
`retry_total`, skipped-by-size, folder-error, and duration fields.

### Agent-facing documentation

Shared repository instructions live in [`AGENTS.md`](AGENTS.md). The canonical
installable operator skill is a single file,
[`skills/gdstt-cli/SKILL.md`](skills/gdstt-cli/SKILL.md).
...
Copilot, Windsurf, and many more. Install the canonical package for each host that
should use it; `gh skill` chooses the host-specific destination.
...
After changing the canonical package, validate it with:
