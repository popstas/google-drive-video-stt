...
def test_transcribe_file_deepgram_full_file(mocker, tmp_path):
    ...
    provider = MagicMock(spec=DeepgramProvider)
...
def test_transcribe_file_non_deepgram_provider_skips_cost(mocker, tmp_path):
    """A provider that is not a DeepgramProvider must not trigger cost lookup."""
    mp3 = tmp_path / "a.mp3"
    mp3.write_bytes(b"x")

    provider = MagicMock()  # deliberately not spec'd as DeepgramProvider
    provider.transcribe_full.return_value = "[00:00:00] Speaker 1: text"
    mocker.patch("src.stt.transcribe.get_provider", return_value=provider)
    fetch_mock = mocker.patch("src.stt.transcribe.fetch_request_cost_usd")
