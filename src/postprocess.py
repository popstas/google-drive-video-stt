...
# "Ольга х ExpertizeMe", "Olga x ExpertizeMe"). Both the cyrillic "х" and the latin "x" are
# matched case-sensitively: an uppercase "X"/"Х" is a middle initial ("Malcolm X and Alice",
# "Малкольм Х и Алиса"), not a separator, and matching it would split the name and strand
# the real conjunction.
_NAME_SEP_RE = re.compile(
    r"\s*,\s*|\s+&\s+|\s+and\s+|\s+и\s+|(?-i:\s+х\s+|\s+x\s+)", re.IGNORECASE
)
