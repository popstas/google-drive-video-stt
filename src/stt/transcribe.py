...
def transcribe_file(
    ...
    *,
    ...
    # Deepgram is the only non-empty provider, so a non-empty stt_provider always
    # means an isinstance(provider, DeepgramProvider) here.
    if isinstance(provider, DeepgramProvider):
...
def _log_deepgram_cost(
    ...
    duration_part = "" if duration is None else f", duration={duration:.2f}s"
