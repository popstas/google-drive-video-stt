...
(`Speaker 1`, `Speaker 2`, ...) на реальные имена/wikilink и объединяет подряд идущие
реплики одного говорящего в блок. Содержимое слов реплики сохраняется - скрипт не
переписывает и не сокращает сказанное; но это НЕ побайтовое сохранение: пробелы
нормализуются (строки .strip()-аются и склеиваются одним пробелом), а заголовок
и таймкоды переформатируются. То есть сохраняются слова, нормализуются пробелы,
склеиваются подряд идущие реплики одного говорящего.

Поддерживаемые форматы строк:
  - диаризация:  [HH:MM:SS] Speaker 1: текст        (одна реплика = одна строка)
  - krisp-стиль: **Имя | 01:23**                    (метка+таймкод, текст на следующих строках)
                 текст реплики...
...
Формат MAP.json:
{
  ...
  "default": { "Speaker 1": "[[Имя1]]",
               "Speaker 2": "[[Имя1]]",
               "Speaker 3": "Имя2" },
  "exceptions": [ { "text": "Алло, привет.", "name": "Имя2" } ]
}
...
def mmss(h, m, s):
    """Нормализовать таймкод, не теряя часы.

    Регулярки отдают группы (h, m, s), где s может быть None:
      - две группы (s is None) -> исходник был MM:SS, т.е. h=минуты, m=секунды;
        возвращаем "MM:SS".
      - три группы -> исходник был HH:MM:SS; если часов нет (h == 0),
        возвращаем "MM:SS", иначе "HH:MM:SS". Часы НЕ сворачиваются в минуты,
        чтобы [01:30:00] не превращалось в неоднозначное "90:00".
    """
    if s is None:
    ...
    if has_krisp:
        ...
        for ln in lines:
            ...
            if m:
                ...
                stripped = ln.strip()
                # Сохраняем ЛЮБОЕ непустое содержимое реплики, включая одинокие
                # числа ("42") и строки, начинающиеся с '#'. Раньше такие строки
                # молча выбрасывались, что теряло реальные слова говорящего.
                # Единственный отбрасываемый артефакт krisp-экспорта - пустые
                # строки (нормализация пробелов при склейке реплики).
                if stripped:
...
def relabel(src_text, map_cfg, *, include_header=True):
    ...
    Слова реплики сохраняются (без переписывания/сокращения), но пробелы
    нормализуются, а метка и группировка меняются - это не побайтовое сохранение.
