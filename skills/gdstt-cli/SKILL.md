...
`gdstt planfix sent` - записи, чей комментарий реально ушёл, блоком на запись:
время, менеджер, ссылка на задачу (из `planfix.task_url`), ссылка на Drive, имя. Маркер
`planfix_comment_task_id` пишется на MP4 только после успешного POST, так что это
журнал отправленного - в отличие от `bookings list` с полученными бронями (бронь
может быть, а комментарий не уйти). Только чтение.
