...
Ярлык (shortcut) на запись не видит ни один путь: Drive отдаёт
`application/vnd.google-apps.shortcut`, настоящий тип лежит в `shortcutDetails`.
Так и задумано - у организатора настоящий файл, а обработка ярлыка у участника
расшифровала бы тот же звонок второй раз. И это не редкость: в первой проверенной
папке сотрудника три встречи из одиннадцати были только ярлыками. Ярлык остаётся
ярлыком при любом доступе, доступ решает лишь, откроется ли запись. `doctor --drive`
называет, сколько таких звонков папка не обработает и сколько записей не открывает этот аккаунт.
...
### `meet mark <show|reset>`

`run.discovery: meet` - искать работу не обходом Drive, а вопросом к Meet: в каких
созвонах человек был. Один запрос на сотрудника за цикл при любом размере архива, в
ответе сразу id файла записи. Обход стоит запрос на каждую папку встречи (замерено: 10
сотрудников, 171 встреча - 211 запросов за цикл) и растёт с архивом.

Нужен сервисный аккаунт: Meet отвечает только за того, кто спрашивает, поэтому конфиг
без ключа этот режим не загружает. Скоуп один - `meetings.space.readonly`; сам Meet API
должен быть включён в проекте сервисного аккаунта, иначе 403 `SERVICE_DISABLED` (это
про нашу консоль, не про чужие права).

`meet_checked_at.txt` - момент, до которого всё доделано; читаемое время, а не курсор,
потому что Meet фильтрует созвоны по началу. **Через недоделанное отметка не переходит.**
Meet называет созвон сразу после конца, файл появляется через 5-8 минут (замерено),
поэтому отметку держат идущий созвон, запись без файла и созвон с непрочитанными
записями. Закончившийся созвон без записей не держит ничего, любое
другое держание снимается через `meet.wait_hours` (24) со строкой в логе. Запись,
которой владеет никто из `folders:`, не берётся в работу и считается отдельно - обход
её тоже не берёт (у участника только ярлык). Двигается отметка, как и
курсор, только после цикла, который всё выгреб (иначе `Holding the Meet mark [...]`).

Нет файла - смотрим назад на `meet.first_look_hours` (168) и не дальше `run.since`; что
вообще в работе, решает только `run.since`. `meet.fallback: walk|none` - сотрудника, на
котором Meet отказал, этим циклом обойти (по умолчанию) или просто посчитать ошибкой;
отметка в обоих случаях стоит на месте. `show` печатает её, `reset` забывает - после
сброса цикл смотрит на окно первого взгляда, а не на весь архив.

Обход и журнал остаются рабочими: `run-once --mode meet` и `--mode walk` на одном
моменте обязаны найти одни и те же записи, расхождение - баг.

### Что Meet знает про сам созвон

В режиме `meet` вместе с записью приходит и то, кто был в созвоне, когда зашёл и
вышел, кто говорил и когда звонок реально начался. В `walk` ничего из этого нет, и всё
ниже ведёт себя как раньше.

**Никто не пришёл - не расшифровываем.** Правило: двое ни разу не были в созвоне
одновременно. Любое пересечение, хоть секунда, считается «пришёл» - у настоящих
звонков замерено пересечение от 15 секунд, порог выбрасывал бы реальные разговоры.
Рядом с видео появляется `<имя>.skipped` с текстом, кого видели и почему ничего не
сделали; удалить файл и `reprocess` - расшифрует. Нечитаемое присутствие пропуском не
считается никогда. Выключается `meet.skip_empty_calls: false`.

**Участники в промптах.** `{{participants}}` - все, кто был, `{{participants-speakers}}`
- только говорившие. Подставляются в момент отправки промпта, а не при загрузке
конфига (в отличие от `{{entities}}`). Плейсхолдер, которому нечего сказать, уносит
свою строку целиком. «Кто говорил» стоит двух лишних запросов на запись, поэтому
запрашивается только если такой плейсхолдер реально есть в промпте.

**Время созвона берётся у Meet**, а не разбирается из имени файла. Это убирает отказ
`no-meeting-time` у звонков, названных по комнате, а не по событию календаря.

**Кандидаты для определения спикеров** собираются из обоих источников сразу. Первыми
идут имена из документа Meet - они дословно те же строки, что модель видит в репликах-
уликах, а ответ проверяется по этому списку, и другое написание означало бы выброшенный
верный ответ. Дальше добавляются участники из API: документ читается с лимитом в два
имени, а звонок вчетвером - обычное дело. Говорившие идут раньше просто присутствовавших. Определять спикеров по
таймингам Meet пробовали тремя способами, все три провалились -
`docs/reports/2026-09-19-presence-speakers.md`.

Участник приходит с отображаемым именем и непрозрачным id, **без адреса** - сопоставить
их нечем, человек опознаётся по имени.
...
### `planfix sent` / `telegram sent`

`gdstt planfix sent [--limit N]` - записи с ушедшим комментарием; `gdstt telegram sent
[--limit N]` - со сводкой хоть в одном чате. Блок на запись, новые сверху (`--limit 0` -
все): время, менеджер, задача (`planfix.task_url`) или чаты, ссылка на подпапку звонка
(иначе на видео), имя. Маркеры `planfix_comment_task_id` / `telegram_sent_chat_id` пишутся
только после успешной отправки - журнал отправленного, не броней. Только чтение.

### `doctor [--drive]`

Показать путь к `config.yml`, наличие credentials/token, источник Google-auth
(без секретов), папки с сотрудником (`<folder-id>: Имя <email>`) и их чатами Telegram,
